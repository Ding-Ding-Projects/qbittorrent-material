...
{
    qCDebug(lcRss) << "Constructing RSS feed" << url << "uid=" << uid.toString() << "path=" << path;
...
Feed::~Feed()
{
    qCDebug(lcRss) << "Destroying RSS feed" << m_url;
    store();
...
{
    qCDebug(lcRss) << "Marking feed as read:" << m_url << "unread=" << m_unreadCount;
...
void Feed::refresh()
{
    if (!m_isInitialized)
    {
        qCDebug(lcRss) << "Feed not yet initialized, deferring refresh:" << m_url;
    ...
    qCDebug(lcRss) << "Refreshing RSS feed:" << m_url;
...
void Feed::handleIconDownloadFinished(const Net::DownloadResult &result)
{
    ...
    {
        qCDebug(lcRss) << "RSS feed icon downloaded for" << m_url;
...
void Feed::handleDownloadFinished(const Net::DownloadResult &result)
{
    ...
    if (result.status == Net::DownloadStatus::Success)
    {
        qCInfo(lcRss).noquote() << tr("RSS feed at '%1' is successfully downloaded. Starting to parse it.")
                .arg(result.url);
    ...
    else
    {
        ...
        qCWarning(lcRss).noquote() << tr("Failed to download RSS feed at '%1'. Reason: %2")
                .arg(result.url, result.errorString);
...
void Feed::handleParsingFinished(const RSS::Private::ParsingResult &result)
{
    ...
    if (m_hasError)
    {
        qCWarning(lcRss).noquote() << tr("Failed to parse RSS feed at '%1'. Reason: %2").arg(m_url, result.error);
    }
    qCInfo(lcRss).noquote() << tr("RSS feed at '%1' updated. Added %2 new articles.")
...
void Feed::load()
{
    qCDebug(lcRss) << "Loading persisted articles for feed" << m_url;
...
void Feed::store()
{
    ...
    qCDebug(lcRss) << "Storing" << m_articles.size() << "article(s) for feed" << m_url;
...
void Feed::downloadIcon()
{
    ...
    qCDebug(lcRss) << "Downloading RSS feed icon:" << iconUrl;
...
void Feed::setRefreshInterval(const std::chrono::seconds refreshInterval)
{
    ...
    qCInfo(lcRss) << "Feed refresh interval changed to" << static_cast<qint64>(refreshInterval.count())
                  << "seconds for" << m_url;
...
{
    qCDebug(lcRss) << "Feed URL updated from" << m_url << "to" << url;
...
void Feed::handleArticleLoadFinished(QList<QVariantHash> articles)
{
    ...
    qCDebug(lcRss) << "Feed initialized:" << m_url << "articles=" << m_articlesByDate.size()
                   << "unread=" << m_unreadCount;
    emit stateChanged(this);
...
void Feed::cleanup()
{
    qCDebug(lcRss) << "Cleaning up feed data files for" << m_url;
    m_dirty = false;
    m_savingTimer.stop();
